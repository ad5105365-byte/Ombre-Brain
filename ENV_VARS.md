...
| `OMBRE_GATE_MARGIN` | `0.04` | ③语义门：亲密种子要比中性锚点高出多少才算命中（semantic_gate.py） |
| `OMBRE_GATE_FLOOR` | `0.55` | ③语义门：亲密判定的相似度地板（两边都低时不命中） |
| `OMBRE_FRICTION_MARGIN` | `0.06` | ②摩擦门：凶我/冷淡种子的赢面要求（比亲密门严——它点亮情绪，宁漏勿错） |
| `OMBRE_FRICTION_FLOOR` | `0.60` | ②摩擦门：相似度地板 |
| `OMBRE_PORT_NAME` | 无 | 客户端钩子环境变量：常驻窗口的渡口窗口名（如 `VPS常驻`）；没配用 session_id 前 8 位 |

语义门阈值怎么拧：跑几轮真实对话后看 `/hook-log` 里 recall 条目的 sim 读数
（`[i] c=0.55 h=0.52 i=0.71 n=0.58` = cold/harsh/intimate/neutral 各组最高相似度，
方括号里是命中标志），误放就抬 MARGIN/FLOOR，漏判就降。
