...
输出格式（自然中文段落，禁止 JSON、禁止代码块、禁止 markdown 列表）：
先用一两句话概括核心，再紧凑铺陈全部核心事实与关键原话；
如有情绪状态，用"情绪："带出；如有待办，最后用"待办："列出、分号分隔。
整体控制在 200 字以内。与原文风格一致的短内容直接保留原文措辞。"""

# 摘要风格版本号——掺进缓存键里。改了 DEHYDRATE_PROMPT 的输出风格必须
# 一起升版本，否则旧 JSON 摘要从缓存里回来，注入格式又劈叉成两种。
# Style version salted into the cache key: bump it whenever the prompt's
# OUTPUT STYLE changes, or stale JSON summaries keep leaking from cache.
DEHYDRATE_STYLE_VERSION = "prose-v2"
...
8. 在 content 中对人名、地名、专有名词用 [[双链]] 标记（如 [[张三]]、[[Obsidian]]），普通词汇不要加。注意：[[张三]] 只是格式示范，绝不要把它当成真实人名写进内容里
...
3. 保留所有重要事实
4. 总长度尽量不超过旧记忆的 120%
5. 对出现的人名、地名、专有名词用 [[双链]] 标记（如 [[张三]]、[[Obsidian]]），普通词汇不要加。注意：[[张三]] 只是格式示范，绝不要把它当成真实人名写进内容里
...
class Dehydrator:
    ...
    def _cache_key(self, content: str) -> str:
        """Content hash salted with the summary style version.
        缓存键掺入风格版本号，换风格自动整体失效、后台重新焐热。"""
        return hashlib.sha256(
            (DEHYDRATE_STYLE_VERSION + "\n" + content).encode()
        ).hexdigest()
